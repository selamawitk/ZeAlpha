...
import { CheckCircle, Heart, Share2, Download, ArrowLeft, Gift, PartyPopper, Camera, Sparkles } from 'lucide-react';
...
const ThankYou = () => {
  ...
  const launchConfetti = useCallback(() => {
    ...
    const frame = () => {
      confetti({
        ...
        spread: 55,
        origin: { x: 0 },
        colors: ['#B8860B', '#D4AF37', '#C49B52', '#FFD700'],
      });
      confetti({
        particleCount: 3,
        angle: 120,
    ...
    setTimeout(() => {
      confetti({
        ...
        spread: 100,
        origin: { y: 0.6 },
        colors: ['#B8860B', '#D4AF37', '#C49B52', '#FFD700', '#8B5A00'],
      });
    }, 500);
  }, [confettiLaunched]);
  ...
  const handleShare = async () => {
    const shareData = {
      ...
      text: `I just contributed to "${contribution?.gift?.name || 'a wedding gift'}" on ZeAlpha! 🎉`,
  ...
  if (cancelled) {
    return (
      <motion.div
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
        className="relative min-h-screen flex items-center justify-center bg-gradient-to-br from-[#f7f2ea] via-[#f9f5ef] to-[#eadfce] px-4"
      >
        <motion.div
          initial={{ scale: 0.9, opacity: 0, y: 20 }}
          animate={{ scale: 1, opacity: 1, y: 0 }}
          transition={{ type: 'spring', stiffness: 120, damping: 14 }}
          className={`w-full max-w-md ${glassCard} p-8 text-center`}
        >
          ...
          <Link to="/" className={`mt-6 inline-flex rounded-2xl ${goldGradient} px-6 py-3 text-sm font-bold text-white shadow-lg transition-all hover:brightness-110`}>
            Back to Home
          </Link>
        </motion.div>
      </motion.div>
    ...
    <div className="relative min-h-screen overflow-hidden bg-gradient-to-br from-[#faf6f0] via-[#f8f3eb] to-[#efe2d1] px-4 py-12">
      ...
      <div className="pointer-events-none fixed inset-0 overflow-hidden">
        ...
        <motion.div
          ...
          className="absolute top-[15%] right-[8%] h-8 w-8 text-[#D4AF37]/30"
        >
          <Sparkles />
        </motion.div>
        <motion.div
          ...
          className="absolute bottom-[20%] left-[5%] h-6 w-6 text-[#D4AF37]/20"
        >
          <Heart />
        </motion.div>
      </div>
        ...
        {loading ? (
          <div className="flex items-center justify-center py-24">
            <motion.div
              ...
              className="h-10 w-10 rounded-full border-3 border-[#B8860B] border-t-transparent"
            />
          </div>
        ) : (
          <motion.div variants={containerVariants} initial="hidden" animate="visible">
            ...
            >
              ...
              <motion.div
                ...
                animate="animate"
                className="absolute top-4 right-8 text-[#D4AF37]/20"
              >
                <Sparkles size={28} />
              </motion.div>
              <motion.div
                ...
                animate="animate"
                custom={1}
                className="absolute bottom-8 left-8 text-[#D4AF37]/15"
              >
                <PartyPopper size={24} />
              </motion.div>
                ...
                >
                  ...
                  <div className="mt-4 space-y-3 text-left">
                    <div className="flex justify-between border-b border-[#eedcc9]/50 pb-2">
                      ...
                      <span className="text-sm font-bold text-[#2d2218]">{contribution.gift?.name || 'Wedding Gift'}</span>
                    </div>
                    {contribution.contribution?.amount && (
                      <div className="flex justify-between border-b border-[#eedcc9]/50 pb-2">
                        <span className="text-sm text-[#6f6257]">Amount</span>
                        <span className="text-lg font-black text-[#8B5A00]">ETB {contribution.contribution.amount.toLocaleString()}</span>
                      </div>
                    )}
                    {contribution.contribution?.paymentMethod && (
                      <div className="flex justify-between border-b border-[#eedcc9]/50 pb-2">
                        <span className="text-sm text-[#6f6257]">Method</span>
                        <span className="text-sm font-bold text-[#2d2218] capitalize">{contribution.contribution.paymentMethod}</span>
                      </div>
                    )}
            ...
            >
              <Link
                to={user ? '/my-gifts' : '/'}
                className={`inline-flex items-center gap-2 rounded-2xl ${goldGradient} px-6 py-3.5 text-sm font-bold text-white shadow-lg shadow-[#8B5A00]/20 transition-all duration-300 hover:brightness-110 hover:shadow-xl active:scale-95`}
              >
              ...
              <button
                onClick={handleShare}
                className="inline-flex items-center gap-2 rounded-2xl border border-[#dcc6a7] bg-white/60 px-6 py-3.5 text-sm font-bold text-[#6f6257] transition-all duration-300 hover:bg-white hover:shadow-md hover:border-[#B8860B]/30 active:scale-95"
              >
